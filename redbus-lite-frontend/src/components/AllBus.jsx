...
const AllBus = () => {
  ...
  // NEW: State to store the coordinates of the clicked card
    ...
    <div className="w-full max-w-[1000px] py-10 px-5 relative">
      ...
      <div className="flex flex-col gap-4 overflow-x-auto pb-4">
        {loading ? (
          ...
          buses.map(bus => (
            <div 
              // Capture the bounding rect on click!
              onClick={(e) => {
      ...
      <Modal 
        ...
        title="Route Details"
        originRect={clickRect} // Pass the captured coordinates to the modal
      >
        {selectedBus && (
          <div className="flex flex-col gap-5 text-white">
            <div className="flex items-center justify-between bg-[#1a1a1a] p-4 rounded-xl border border-[#2a2a2a]">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-[#a0a0a0] uppercase tracking-wider mt-1">From</span>
              </div>
              ...
              <div className="flex flex-col text-right">
                ...
                <span className="text-xs text-[#a0a0a0] uppercase tracking-wider mt-1">To</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col p-3 bg-[#1e1e1e] rounded-lg border border-[#2a2a2a]">
                <span className="text-xs text-[#a0a0a0] mb-1">Total Capacity</span>
                <span className="font-medium">{selectedBus.capacity} Seats</span>
              </div>
              <div className="flex flex-col p-3 bg-[#1e1e1e] rounded-lg border border-[#2a2a2a]">
                <span className="text-xs text-[#a0a0a0] mb-1">Availability</span>
                <span className={`font-semibold ${selectedBus.available_seats > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                  {selectedBus.available_seats} Left
                </span>
              </div>
            </div>
